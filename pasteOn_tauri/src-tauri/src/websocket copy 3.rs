...
async fn handle_connection222(socket: WebSocket, clients: Clients) {
    ...
    while let Some(result) = socket_lock.next().await {
        match result {
            Ok(message) => {
                if message.is_text() || message.is_binary() {
                    ...
                    for (_client_id, client) in clients_lock.iter() {
                        ...
                        if Arc::ptr_eq(&socket, &client) {
                            println!("Skipping self-locking to avoid deadlock.");
                            continue; // 避免自锁导致死锁
                        }
