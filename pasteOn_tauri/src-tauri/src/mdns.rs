...
pub async fn register_mdns_service() {
    let mut registered = SERVICE_REGISTERED.lock().await;
    if !*registered {
        *registered = true;
        thread::spawn(|| {
            let mdns = ServiceDaemon::new().expect("Failed to create daemon");
            ...
            // Block the thread indefinitely without sleeping for a fixed time
            loop {
                thread::sleep(Duration::from_secs(3600)); // Sleep for 1 hour before looping again
            }
        });
    } else {
